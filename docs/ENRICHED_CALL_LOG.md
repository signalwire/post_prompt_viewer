...
**`hearing_hint`** — Hearing hints rewrote user input. Only logged when the text actually changed.
...
**`pronounce_rule`** — Pronounce rules rewrote output before TTS. Only logged when the text actually changed.

| Field | Type | Description |
|-------|------|-------------|
| `original` | string | Output text before rewriting |
| `result` | string | Output text after rewriting |

**`auto_correct`** — LLM-based auto-correction rewrote user input. Only logged when the text actually changed.
...
**`text_normalize`** — FST-based text normalization (NeMo) rewrote text. Only logged when the text actually changed.

| Field | Type | Description |
|-------|------|-------------|
| `direction` | string | `"itn"` (inverse text normalization on ASR input) or `"tn"` (text normalization on TTS output) |
| `original` | string | Text before normalization |
...
These are unchanged structurally. Metadata is flattened to top level in blessed output (same as before). User entries have `confidence`, `start_timestamp`, `end_timestamp`, timing fields. Assistant entries have `latency`, `utterance_latency`, `audio_latency`, and `acoustic_latency`.
...
Wall-clock μs timestamps exposed on the assistant entry alongside the pre-computed latencies. Use these when you need a measurement the pre-computed fields don't cover (e.g. subtracting from any other wall-clock event in your stack). All three come from mod_deepgram's final result JSON; absent in OART mode or when mod_deepgram didn't supply them.
...
```json
{
  "call_timeline": [
    ...
    {"ts": 1705300003789000, "type": "user_input", "confidence": 95.5, "start_timestamp": 1705300002000000, "end_timestamp": 1705300003789000},
